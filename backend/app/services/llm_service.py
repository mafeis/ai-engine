...
【重要提示】：每个角色（主角/NPC/敌人）的外观描述必须结合“游戏类型”及其特色，并请将其总结为一句简单、精准、不超过 50 个字的自然语言描述，这将直接用于后续 AI 生成资源。""",
...
请创建包含以下内容的 Markdown 文档:
1. 场景列表: 每个场景需包含：
   - 名称与基本描述（结合游戏类型，总结为一句简练、不超过 50 字的描述）
   - 场景总尺寸 (如 1920x1080)
...
【重要提示】：场景的“基本描述”必须结合游戏的“美术风格”和“类型”，并保持极简（50字以内）。""",
...
【重要提示】：每个道具的外观描述必须结合“游戏类型”和“美术风格”，并请将其总结为一句简单、精准、不超过 50 个字的自然语言描述。""",
...
【重要提示】：每个 UI 元素或界面的描述需结合游戏类型，并总结为一句不超过 50 字的内容。""",
...
【重要提示】：每个音频的描述需结合游戏背景及其类型，并保持简练（不超过 50 字）。"""
        }
...
1. 所有字段都有合理的值。
2. appearance/description 字段必须【强制】结合该游戏的类型和风格特色，将其总结为一句简单、精准、绝对不超过 50 个字的自然语言描述。
3. 数值字段使用合理的游戏数值。"""
...
代码质量: 包含完整的 import，添加详细中文注释。确保代码在 `if __name__ == '__main__': main()` 中执行。"""
            ...
            prompt = f"""请生成一个【极其精简且高效】的 Python 脚本，使用 PIL (Pillow) 库绘制以下图像资源。

资源类型: {resource_type}
描述: {description}
参数: 尺寸 {params.get('size', 64)}x{params.get('size', 64)}，风格 {params.get('style', 'pixel')}

【核心生成要求】：
1. **模块化设计**: 定义一个核心 `draw_base(draw, color_indices, offsets)` 函数。严禁为相似的部分重复编写绘图代码。
2. **高效循环**: 使用循环来绘制重复的像素点或纹理，而非手动列举每一行代码。
4. **严格长度控制**: 整个脚本必须控制在 100 行以内。如果代码过长将导致生成失败。
5. **填充率要求**: 主体物品必须【填满】画布。严禁在四周留下过大的空白。物品比例应占据至少 85% 的画布区域。
6. **接口规范**: 必须接受 `--output` 和 `--seed` 参数。使用 `random.seed(args.seed)` 确保变体多样性。
7. **视觉逻辑**: 
   - 绘制基础剪影。
   - 绘制核心特征（如角色的眼睛、衣服，或场景的核心物件）。
   - 增加简单的明暗噪点以符合像素风格。
   - 背景必须透明 (RGBA)。
...
请根据描述生成完整的音频生成代码。"""
    ...
    async def generate_animation_script(
        ...
        """
        prompt = f"""请生成一个【高复用性、精简】的 Python 脚本，使用 PIL 库绘制角色的【序列帧 Spritesheet】。

角色 ID: {resource_id}
角色描述: {description}
单帧尺寸: {params.get('size', 64)}x{params.get('size', 64)}

【强制要求】：
1. **高度复用代码**: 
   - 必须定义一个 `draw_character(draw, x_offset, y_offset, pose_params)` 函数。
   - 所有的动画帧（Idle, Walk, Attack）必须通过循环调用此函数并传入不同的偏移/姿态参数来绘制。
   - **严禁**为每一帧编写独立的绘图逻辑！
2. **Spritesheet 布局**: 输出为 4x3 的网格 (RGBA)。第1行待机，第2行行走，第3行攻击。
3. **极致精简**: 整个脚本代码量必须尽可能小（建议 120 行内），防止 LLM 生成截断。
4. **填充率关键要求**: 主体角色必须尽可能【填满】单帧画布。严禁在四周留下过大的空白。角色核心躯干高度应占据至少 85% 的画布高度。例如 64x64 画布，角色头顶应在 y=2 附近，脚部在 y=62 附近。
5. **接口**: 接收 `--output` 参数。

绘图建议：使用简单的几何图形组合来构建角色外观。"""
