    ...
    projectPanel.innerHTML = `
        ...
        <div class="section">
            ...
            <div class="resource-actions">
                ...
                <div class="resource-type-card clickable" onclick="showResourcePanel('${project.id}', 'audio')">
                    <span class="icon">🔊</span>
...
function renderVariantsHtml(result, projectId, specType, itemId) {
    ...
    if (selectedVariant) {
        ...
        html += '<div class="selected-main-card">';
        html += '   <div class="selected-badge">✓ 已选定方案</div>';
...
async function generateCharacterAnimations(projectId, itemId) {
    ...
    try {
        ...
        if (result.success) {
            alert('✓ 序列帧动画设计完成！\n预览地址：' + result.spritesheet_url);
            // 这里可以弹出一个预览，或者简单通知
            window.open(`${API_BASE.replace('/api', '')}${result.spritesheet_url}`, '_blank');
        }
    } catch (error) {
